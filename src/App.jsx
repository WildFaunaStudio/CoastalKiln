...
import { Plus, Home, Settings, Flame, Palette, ArrowLeft, Camera, CheckCircle, Trash2, X, Leaf, ChevronDown, ChevronUp, Users, MessageSquare, Calendar, BookOpen, Send } from 'lucide-react';
...
function CoastalKilnApp() {
  const [user, setUser] = useState(() => storage.get('user', { username: 'Daniel', email: 'potter@coastalkiln.com', bio: 'Potter & ceramic artist', location: 'Wellington, NZ' }));
  ...
  const [form, setForm] = useState({ title: '', clay: '', name: '', type: '', recipe: '', photo: null, weight: '', source: '', batchNotes: '', tipCategory: 'clay_reclaim', tipTitle: '', tipContent: '', tipTags: [], guildName: '', guildLocation: '', guildDesc: '', inviteCode: '', guildPost: '', resourceTitle: '', resourceType: 'PDF' });
  ...
  const stageColors = {
    ...
    bisque: 'bg-cyan-100 text-cyan-800', glazing: 'bg-blue-100 text-blue-800',
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-teal-50 pb-20">
      {/* Header */}
      <header className="sticky top-0 z-40 bg-gradient-to-br from-slate-50 to-teal-50">
        <div className="max-w-7xl mx-auto px-4 h-16 flex justify-between items-center">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 bg-gradient-to-br from-teal-500 to-cyan-600 rounded-lg flex items-center justify-center text-2xl">🌊</div>
            <div>
          ...
          <button onClick={() => setShowSettings(true)} className="p-2 text-slate-600 hover:bg-white/50 rounded-lg">
            <Settings size={20} />
          </button>
        </div>
      </header>
        ...
        {currentView === 'sustainable' && selectedBatch ? (
          ...
          <div className="space-y-4">
            ...
            <div className="bg-white rounded-lg p-6 shadow-sm">
              ...
              <div className="mb-4">
                ...
                }} placeholder="Optional" className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-teal-500" />
              </div>
              ...
              <div>
                ...
                }} placeholder="Clay type, mixing notes..." rows={4} className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-teal-500" />
              </div>
            </div>
            ...
            <div className="flex gap-3">
              ...
              <button onClick={() => setSelectedBatch(null)} className="flex-1 px-4 py-3 bg-teal-600 text-white rounded-lg hover:bg-teal-700 font-medium">
                Save
              </button>
            </div>
          </div>
          ...
          <div className="space-y-6">
            ...
            <div className="flex gap-2 border-b border-slate-200">
              <button onClick={() => setSustainableTab('reclaim')} className={`px-4 py-2.5 font-medium border-b-2 transition-colors ${sustainableTab === 'reclaim' ? 'border-teal-600 text-teal-700' : 'border-transparent text-slate-600'}`}>
                Clay Reclaim
              </button>
              <button onClick={() => setSustainableTab('tips')} className={`px-4 py-2.5 font-medium border-b-2 transition-colors ${sustainableTab === 'tips' ? 'border-teal-600 text-teal-700' : 'border-transparent text-slate-600'}`}>
                Studio Tips
              </button>
            </div>
                ...
                {['clay_reclaim', 'diy_tools', 'plaster_bats'].map(category => {
                  ...
                  return (
                    <div key={category} className="bg-white rounded-lg overflow-hidden shadow-sm">
                      <button onClick={() => setExpandedTips(p => ({ ...p, [category]: !p[category] }))} className="w-full flex items-center justify-between p-4 hover:bg-slate-50 transition-colors">
                      ...
                      {expandedTips[category] && (
                        <div className="p-4 pt-0 space-y-3">
                          {categoryTips.map(tip => (
                            <div key={tip.id} className="bg-slate-50 rounded-lg p-4">
                              ...
                              <div className="flex flex-wrap gap-2">
                                {tip.tags.map((tag, idx) => (
                                  <span key={idx} className="px-2 py-1 bg-teal-100 text-teal-800 text-xs rounded-full">{tag}</span>
                                ))}
                              </div>
                            </div>
                          ))}
          ...
          <div className="space-y-4">
              ...
              {selectedGuild.isAdmin && (
                <div className="bg-slate-50 rounded-lg p-4">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button onClick={() => { navigator.clipboard.writeText(selectedGuild.inviteCode); alert('Copied!'); }} className="px-4 py-2 bg-teal-600 text-white rounded-lg hover:bg-teal-700 text-sm">
                      Copy
                    </button>
                  </div>
            ...
            {selectedGuild.event && (
              <div className="bg-teal-50 rounded-lg p-6 shadow-sm">
                <div className="flex items-start gap-3">
                  <Calendar className="text-teal-600 mt-1" size={24} />
                  <div>
                    <h3 className="font-semibold text-teal-900 mb-1">Upcoming Event</h3>
                    <p className="text-teal-800">{selectedGuild.event}</p>
                  </div>
                </div>
              </div>
            )}
            ...
            <div className="bg-white rounded-lg p-6 shadow-sm">
              ...
              <button onClick={() => setShowModal(true)} className="w-full px-4 py-2 bg-teal-600 text-white rounded-lg hover:bg-teal-700">
                New Post
              </button>
            </div>
          ...
          <div className="space-y-6">
            ...
            <div className="flex gap-2 border-b border-slate-200">
              <button onClick={() => setGuildTab('my-guilds')} className={`px-4 py-2.5 font-medium border-b-2 transition-colors ${guildTab === 'my-guilds' ? 'border-teal-600 text-teal-700' : 'border-transparent text-slate-600'}`}>
                My Guilds
              </button>
              <button onClick={() => setGuildTab('discover')} className={`px-4 py-2.5 font-medium border-b-2 transition-colors ${guildTab === 'discover' ? 'border-teal-600 text-teal-700' : 'border-transparent text-slate-600'}`}>
                Discover
              </button>
            </div>
            ...
            {guildTab === 'my-guilds' ? (
                ...
                {guilds.filter(g => g.isMember).map(guild => (
                  <button key={guild.id} onClick={() => setSelectedGuild(guild)} className="w-full bg-white rounded-lg p-6 hover:shadow-md transition-shadow text-left shadow-sm">
                    <div className="flex justify-between items-start mb-2">
                      ...
                      <span className="px-3 py-1 bg-teal-100 text-teal-800 text-sm font-medium rounded-full">Member</span>
                    </div>
              ...
              <div className="space-y-4">
                <div className="bg-gradient-to-br from-teal-50 to-cyan-50 rounded-lg p-6 shadow-sm">
                  <h3 className="font-semibold text-slate-800 mb-3">Have an Invite Code?</h3>
                  <button onClick={() => setShowModal(true)} className="w-full px-4 py-3 bg-teal-600 text-white rounded-lg hover:bg-teal-700 font-medium">
                    Join by Invite Code
                  </button>
                </div>
          ...
          <div className="space-y-6">
            ...
            <div className="px-2">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <button onClick={() => setTab('glazes')} className={`p-6 rounded-2xl text-left transition-all ${tab === 'glazes' ? 'bg-gradient-to-br from-blue-100 to-teal-100 shadow-md' : 'bg-white shadow-sm'}`}>
                  <div className="w-10 h-10 rounded-full bg-white flex items-center justify-center mb-3">
                    <Palette size={22} className={tab === 'glazes' ? 'text-teal-600' : 'text-slate-600'} />
                  </div>
            ...
            {tab === 'pieces' ? (
              <div className="grid grid-cols-2 gap-3">
                ...
                {projects.length === 0 && (
                  <div className="col-span-2 text-center py-12 bg-slate-50 rounded-lg shadow-sm">
          ...
          <div className="space-y-4">
            ...
            <div className="bg-white rounded-lg border border-slate-200 p-6">
              ...
              <div className="mb-4">
                ...
                <label className="flex items-center justify-center gap-2 px-4 py-3 border-2 border-dashed border-slate-300 rounded-lg text-slate-600 hover:border-teal-500 cursor-pointer">
                  <Camera size={20} />
              ...
              <div>
                ...
                }} placeholder="Add notes..." rows={4} className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-teal-500" />
              </div>
            </div>
          </div>
        ) : (
          ...
          <div className="space-y-4">
            ...
            <div className="bg-white rounded-lg border border-slate-200 p-6">
              ...
              <pre className="text-sm text-slate-700 whitespace-pre-wrap font-mono bg-slate-50 p-4 rounded-lg">{selected.recipe || 'No recipe'}</pre>
            </div>
            ...
            <div className="bg-white rounded-lg border border-slate-200 p-6">
              ...
              }} placeholder="Application tips..." rows={4} className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-teal-500" />
            </div>
          </div>
        )}
      </main>
      ...
      {!selected && !selectedBatch && !selectedGuild && (currentView === 'studio' || currentView === 'sustainable' || currentView === 'guilds') && (
        <button onClick={() => setShowModal(true)} className="fixed bottom-24 right-6 w-14 h-14 bg-teal-600 text-white rounded-full shadow-lg flex items-center justify-center hover:scale-110 z-30">
      ...
      <nav className="fixed bottom-0 left-0 right-0 bg-white border-t border-slate-200 z-40">
        <div className="max-w-7xl mx-auto px-2 flex justify-around h-16">
          <button onClick={() => { setCurrentView('studio'); setSelected(null); setSelectedBatch(null); setSelectedGuild(null); }} className={`flex flex-col items-center justify-center flex-1 ${currentView === 'studio' ? 'text-teal-600' : 'text-slate-500'}`}>
            <Home size={24} strokeWidth={currentView === 'studio' ? 2.5 : 2} />
            <span className="text-xs mt-1 font-medium">Studio</span>
          </button>
          <button onClick={() => { setCurrentView('sustainable'); setSelected(null); setSelectedBatch(null); setSelectedGuild(null); }} className={`flex flex-col items-center justify-center flex-1 ${currentView === 'sustainable' ? 'text-teal-600' : 'text-slate-500'}`}>
            <Leaf size={24} strokeWidth={currentView === 'sustainable' ? 2.5 : 2} />
            <span className="text-xs mt-1 font-medium">Studio Cycle</span>
          </button>
          <button onClick={() => { setCurrentView('guilds'); setSelected(null); setSelectedBatch(null); setSelectedGuild(null); }} className={`flex flex-col items-center justify-center flex-1 ${currentView === 'guilds' ? 'text-teal-600' : 'text-slate-500'}`}>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-lg max-w-md w-full p-6 max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-4">
              ...
              <button onClick={() => { setShowModal(false); setForm({ title: '', clay: '', name: '', type: '', recipe: '', photo: null, weight: '', source: '', batchNotes: '', tipCategory: 'clay_reclaim', tipTitle: '', tipContent: '', tipTags: [], guildName: '', guildLocation: '', guildDesc: '', inviteCode: '', guildPost: '', resourceTitle: '', resourceType: 'PDF' }); }}>
                <X size={24} />
              </button>
            </div>
            ...
            {currentView === 'guilds' && guildTab === 'discover' && !selectedGuild ? (
              <div className="space-y-4">
                ...
                <div className="flex gap-3">
                  ...
                  }} disabled={!form.inviteCode} className="flex-1 px-4 py-2 bg-teal-600 text-white rounded-lg disabled:bg-slate-300">Join</button>
                </div>
              </div>
              ...
              <div className="space-y-4">
                ...
                <div className="flex gap-3">
                  ...
                  <button onClick={() => {
                    ...
                    setShowModal(false);
                  }} disabled={!form.guildPost} className="flex-1 px-4 py-2 bg-teal-600 text-white rounded-lg disabled:bg-slate-300 flex items-center justify-center gap-2">
                    <Send size={18} />
                    Post
                  </button>
                </div>
              ...
              <div className="space-y-4">
                ...
                <div className="flex gap-3">
                  ...
                  <button onClick={() => {
                    ...
                    setShowModal(false);
                  }} disabled={!form.guildName || !form.guildLocation} className="flex-1 px-4 py-2 bg-teal-600 text-white rounded-lg disabled:bg-slate-300">Create</button>
                </div>
              </div>
              ...
              <div className="space-y-4">
                ...
                <div className="flex gap-3">
                  ...
                  <button onClick={() => {
                    ...
                    setShowModal(false);
                  }} disabled={!form.source} className="flex-1 px-4 py-2 bg-teal-600 text-white rounded-lg disabled:bg-slate-300">Create</button>
                </div>
              </div>
              ...
              <div className="space-y-4">
                ...
                <div className="flex gap-3">
                  ...
                  <button onClick={() => {
                    ...
                    setForm({ title: '', clay: '', name: '', type: '', recipe: '', photo: null, weight: '', source: '', batchNotes: '', tipCategory: 'clay_reclaim', tipTitle: '', tipContent: '', tipTags: [] });
                    setShowModal(false);
                  }} disabled={!form.tipTitle || !form.tipContent} className="flex-1 px-4 py-2 bg-teal-600 text-white rounded-lg disabled:bg-slate-300">Add Tip</button>
                </div>
              </div>
            ) : tab === 'pieces' ? (
              <div className="space-y-4">
                {form.photo ? (
                  ...
                  <label className="flex flex-col items-center justify-center aspect-square border-2 border-dashed rounded-lg cursor-pointer hover:border-teal-500">
                ...
                <div className="flex gap-3">
                  ...
                  <button onClick={() => {
                    ...
                    setForm({ title: '', clay: '', name: '', type: '', recipe: '', photo: null, weight: '', source: '', batchNotes: '', tipCategory: 'clay_reclaim', tipTitle: '', tipContent: '', tipTags: [] });
                    setShowModal(false);
                  }} disabled={!form.title || !form.clay} className="flex-1 px-4 py-2 bg-teal-600 text-white rounded-lg disabled:bg-slate-300">Create</button>
                </div>
              </div>
            ) : (
              <div className="space-y-4">
                ...
                <div className="flex gap-3">
                  ...
                  <button onClick={() => {
                    ...
                    setForm({ title: '', clay: '', name: '', type: '', recipe: '', photo: null, weight: '', source: '', batchNotes: '', tipCategory: 'clay_reclaim', tipTitle: '', tipContent: '', tipTags: [] });
                    setShowModal(false);
                  }} disabled={!form.name || !form.type} className="flex-1 px-4 py-2 bg-teal-600 text-white rounded-lg disabled:bg-slate-300">Create</button>
                </div>
              </div>
            )}
          </div>
          ...
          <div className="bg-white rounded-lg max-w-md w-full p-6">
            ...
            <div className="space-y-4">
              <div className="flex flex-col items-center mb-4">
                <div className="w-20 h-20 bg-gradient-to-br from-teal-400 to-cyan-500 rounded-full flex items-center justify-center text-white text-3xl font-bold mb-2">
                  {user.username[0]}
                </div>
              </div>
              <input type="text" value={user.username} onChange={(e) => setUser({ ...user, username: e.target.value })} placeholder="Username" className="w-full px-3 py-2 border rounded-lg" />
              <input type="email" value={user.email} onChange={(e) => setUser({ ...user, email: e.target.value })} placeholder="Email" className="w-full px-3 py-2 border rounded-lg" />
              <textarea value={user.bio} onChange={(e) => setUser({ ...user, bio: e.target.value })} placeholder="Bio" rows={3} className="w-full px-3 py-2 border rounded-lg" />
              <input type="text" value={user.location} onChange={(e) => setUser({ ...user, location: e.target.value })} placeholder="Location" className="w-full px-3 py-2 border rounded-lg" />
              <button onClick={() => setShowSettings(false)} className="w-full px-4 py-2 bg-teal-600 text-white rounded-lg">Save</button>
            </div>
          </div>
        </div>
      )}
